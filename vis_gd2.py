...
#map_file = "log_folder/" + "map_gd2.csv"
inputs = []
outputs = []
plot_folder = "plots/"
...
while len(l)>1:
  ...
  if os.path.isfile(OUTPUT_FILE):
    with open(OUTPUT_FILE) as json_file:
      ...
      if not (len(nodes)==len(pos)):
        Dimension_mismatch_files.append(OUTPUT_FILE)
        continue
...
print(len(Dimension_mismatch_files), "files got dimension mismatch:", Dimension_mismatch_files)
